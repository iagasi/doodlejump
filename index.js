...
function detectWithEnemyCollision(object, target) {
    ...
    if (
        object.position.y + object.height >= target.position.y + target.computedHeight
        &&
...
class Player extends Sprite {
    ...
    detectEnemyCollision() {
        ...
        for (let i = 0; i < arrEnemies.length; i++) {
            ...
            if (detecttWithEnemyTopCollision(this, enm) && this.isFalling && !enm.killed) {
                this.isFalling = false
    ...
    bounce() {
        if (this.colidedStatic && player.position.y < this.colidedStatic.y - this.jumpHeight) {
            this.isFalling = true
    ...
    move() {
        ...
        if (true) {
            if (pressedKeys.w && this.colDetected) { this.position.y -= 3 }
            if (pressedKeys.s && this.colDetected) { this.position.y += 3 }
        }
    }
}
